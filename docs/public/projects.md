---
slug: projects
category: プロジェクト
title: 対応するプロジェクト
description: 送信対象、制限、LuaLaTeX向けの最小構成を説明します。
navOrder: 50
updated: "2026-09-23"
since: "v1.0.0"
---

## 構成

既定のentrypointはプロジェクトのルートにある `main.tex` です。CLIやMCPで明示すれば、Source内の任意の相対 `.tex` パスもentrypointにできます。画像、BibTeXデータ、ローカルのスタイルやクラスファイルは参照関係を保ったまま含めます。

入れ子のentrypointでも、コンパイル時の作業ディレクトリはProjectのルートです。たとえば `docs/main.tex` の `\input{body.tex}` は `project/body.tex` を参照する構成にしてください。`project/docs/body.tex` を参照するなら `\input{docs/body.tex}` と記述します。entrypointのあるフォルダへ自動で移動はしません。

```text
project/
├─ main.tex
├─ references.bib
├─ images/
│  └─ figure.png
└─ styles/
   └─ local.sty
```

## 日本語の最小例

```latex
\documentclass{ltjsarticle}
\begin{document}
日本語とEnglishを含む文書です。
\end{document}
```

レンダラーはLuaLaTeXを使用します。利用可能なパッケージとフォントは配布環境に依存するため、不足が出た場合は `compile.log` のメッセージを管理者へ伝えてください。

Webアプリでは[レンダリング環境](/app/environment/)から、現在のコンテナに入って
いるパッケージとフォントを検索できます。この一覧はデプロイ時に実際のRenderer
イメージから生成され、Remote MCPの環境確認機能と共通です。

## 保存Projectと改訂

[`/app/`](/app/) からアップロードした文書はプロジェクトへ保存されます。同じ
プロジェクトを保存先に選び、Sourceまたはentrypointを変えると不変の改訂が追加され、元のファイル名、entrypoint、
各改訂から作ったJobを確認できます。改訂の「もう一度変換」は保存済みSourceを
使うため、同じファイルを再アップロードしません。同じSource・entrypointなら、
PDF／PDF＋SVGの出力形式だけを変えても改訂は増えません。出力形式と結果は各Jobへ
記録されます。「もう一度変換」の出力形式で指定でき、「初回の設定」を選ぶと
その改訂を初めて作ったときの設定を使います。既存の変換履歴は書き換えません。

保存Projectと改訂履歴は同じユーザーのWeb・CLI・公開API・Local MCP・Remote MCPで共通です。同じユーザーの複数のAPIキー／Service Accountから閲覧・更新できます。Projectに保存しない一時的なSource／Jobのフローも残ります。AIやCLIが保存した改訂もWebに表示され、どの経路からでも固定の改訂IDで再レンダリングできます。

プロジェクトを削除してもJobを即時削除せず、それぞれの既存の保存期限に従います。
アクティブなプロジェクトが参照しているSourceは、再レンダリングできるよう保持
され、Source単体の保存期限を過ぎても再変換できます。保持中のSourceも利用者の
保存容量に算入されます。最後の参照プロジェクトを削除すると保持の保護が外れ、
参照Jobもなく保存期限を過ぎたSourceは定期クリーンアップで回収されます。
プロジェクト削除は記録の非表示であり、入力や成果物の完全削除では
ありません。完全削除が必要な場合は管理者へ相談してください。

## 送信から除外されるもの

`.git`、`.render`、LaTeXが生成した補助ファイルはクライアントが除外します。`node_modules` など、その他の不要なフォルダはプロジェクト外へ移してから実行してください。シンボリックリンクは受け付けません。

ディレクトリから送る場合、未対応の拡張子を持つファイルが残っていると、CLIはアップロード前にそのパスを示して停止します。自動で除外はしません。たとえば `README.md` が不要なら、プロジェクト直下の `.latexrenderignore` に `README.md` を記載してください。送信済みZIPの内容はサーバー側で同じ拡張子規則に従って検証します。

対応する拡張子は `.tex`、`.sty`、`.cls`、`.bib`、`.csv`、`.dat`、`.txt`、`.png`、`.jpg`、`.jpeg`、`.pdf` です。大文字・小文字は区別しません。生成済みの補助ファイルは別途除外対象です。独自のスタイルや文献処理ファイルがこの一覧にない場合は、黙って名前を変更せず管理者へ相談してください。

## 制限

- 最大500ファイル
- ZIP送信サイズは最大20 MiB
- entrypointはSource内の相対 `.tex` パス。省略時はプロジェクト直下の `main.tex`
- シェルエスケープと外部コマンド実行は利用不可
- 処理時間、保存容量、保持期間には管理者設定の上限あり
