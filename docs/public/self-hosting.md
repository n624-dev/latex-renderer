...
このページは[`v1.3.3-rc.6`](https://github.com/n624-dev/latex-renderer/releases/tag/v1.3.3-rc.6)のサーバーbundleを対象にします。バージョンが`-rc.N`のReleaseは指定された検証hostだけへ明示適用し、一般利用者はStableを使用してください。Cloudflare構成に加えて、通常のTLSリバースプロキシとOIDC／ローカルパスワード認証を選択できます。このReleaseは公開後にタグや配布ファイルを差し替えできない設定で固定され、次を含みます。

- `latex-renderer-server-1.3.3-rc.6.tar.gz`
