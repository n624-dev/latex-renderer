...
このページは[`v1.3.4-rc.10`](https://github.com/n624-dev/latex-renderer/releases/tag/v1.3.4-rc.10)のサーバーbundleを対象にします。バージョンが`-rc.N`のReleaseは指定された検証hostだけへ明示適用し、一般利用者はStableを使用してください。Cloudflare構成に加えて、通常のTLSリバースプロキシとOIDC／ローカルパスワード認証を選択できます。このReleaseは公開後にタグや配布ファイルを差し替えできない設定で固定され、次を含みます。

- `latex-renderer-server-1.3.4-rc.10.tar.gz`
...
Cloudflare構成では、保存済みWranglerログイン情報の取得も同じビルド領域から実行し、サービス停止前に公開WorkerとTunnelのrouteを読み取り専用で確認します。変更が必要なrouteは配置後に反映しますが、認証・APIのエラーは停止前に検出します。`Cloudflare credentials unavailable`は必ずしもログイン期限切れを意味せず、pnpmの実行環境の不一致でも発生します。実際の認証状態を確認せずにtokenを再発行したり、設定へ書き込んだりしないでください。
