...
async function versions() {
  ...
  for (let page = 1; ; page += 1) {
    const response = await apiFetch(`${baseUrl}?per_page=100&page=${page}`, {
      headers,
    });
    if (!response.ok)
      throw new Error(
        `GHCR version list failed: ${response.status} ${await response.text()}`,
      );
...
async function deleteVersion(version) {
  ...
  if (!response.ok && response.status !== 404)
    throw new Error(
      `Failed to delete GHCR version ${version.id}: ${response.status} ${await response.text()}`,
    );
}
...
for (const version of list) {
  ...
  if (legacyRuntimeTags.length > 0) {
    // Existing v1.2.x servers may still depend on these old registry Runtimes.
    // Preserve them by default and remove them only after an explicit,
    // post-upgrade manual workflow dispatch.
    if (!purgeLegacyRuntimes) continue;
    if (legacyRuntimeTags.length !== tags.length) continue;
    await deleteVersion(version);
    deleted += 1;
    deletedLegacyRuntimes += 1;
    continue;
  }
