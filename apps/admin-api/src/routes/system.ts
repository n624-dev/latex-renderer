...
export function createSystemRouter(deps:AdminDependencies):Hono{
  ...
  r.patch("/config",async(c)=>{const actor=await requireActor(deps,c,"admin:system:write"),input=parse(z.object({key:z.enum(["max_queue_length","max_user_storage_bytes","source_orphan_retention_minutes"]),value:z.number().int().positive(),reason:z.string().min(1).max(500)}),await c.req.json<unknown>());service.updateConfig(actor,input);return c.json({key:input.key,value:input.value});});
  r.post("/maintenance/:action",async(c)=>{const actor=await requireActor(deps,c,"admin:system:write"),action=parse(z.enum(["enable","disable"]),c.req.param("action"));const body=await c.req.json<unknown>().catch(()=>({}));let mode:MaintenanceMode="normal";if(action==="enable")mode=parse(z.object({mode:maintenanceModeSchema.exclude(["normal"]),reason:z.string().min(1).max(500)}),body).mode;return c.json({mode:service.maintenance(actor,action,mode)});});
