...
export function createServiceAccountsRouter(deps:AdminDependencies):Hono{
  ...
  r.get("/",async(c)=>{await requireActor(deps,c,"admin:service-accounts:read");return c.json(service.list({cursor:c.req.query("cursor"),limit:pageSize(c.req.query("pageSize")),query:c.req.query("query")}));});
  ...
  r.patch("/:id",async(c)=>{const actor=await requireActor(deps,c,"admin:service-accounts:write"),input=parse(z.object({name:z.string().min(1).max(100).optional(),clientType:z.enum(["codex","claude-code","mcp","ci","generic"]).optional()}).strict().refine(v=>v.name!==undefined||v.clientType!==undefined),await c.req.json<unknown>());service.update(actor,c.req.param("id"),input);return c.json({id:c.req.param("id"),updated:true});});
