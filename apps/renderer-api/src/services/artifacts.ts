...
export function artifactResponse(
  ...
  const { row, leaseId } = leased,
    normalized = normalize(row.relative_path);
  if (
  ...
  try {
    storagePath = artifactStoragePath(deps.storageRoot, row);
  } catch (error) {
    deps.database.artifacts.deleteLease(leaseId);
    throw error;
  }
  ...
  return new Response(Readable.toWeb(input) as ReadableStream, {
    headers: {
      "Content-Type": contentType(row.type),
      ...
      "Content-Disposition":
        row.type === "preview"
          ? "inline"
          : `attachment; filename="${name.split("/").at(-1)?.replaceAll('"', "") ?? "artifact"}"`,
      "Cache-Control":
